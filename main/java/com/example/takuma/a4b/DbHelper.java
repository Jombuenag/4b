...
//TABLA ESTUDIANTES
    ...
    public Cursor devolverCicloAlumnos(String ciclo){
        SQLiteDatabase db = (this.getWritableDatabase());
        Cursor cursorDB = db.rawQuery("SELECT * FROM " + TABLAESTUDIANTES + " WHERE " + ESTUDIANTE_CICLO + " = " + "'"+ciclo+"';",null);
        return cursorDB;
    }
    //SELECT CURSO ALUMNOS
    public Cursor devolverCursoAlumno(String curso){
        SQLiteDatabase db = (this.getWritableDatabase());
        Cursor cursorDB = db.rawQuery("SELECT * FROM " + TABLAESTUDIANTES + " WHERE " + ESTUDIANTE_CURSO + " = " + "'"+curso+"';",null);
        return cursorDB;
    }
    //SELECT TUTOR
    public Cursor devolverCursoProfesor(String tutor){
        SQLiteDatabase db = (this.getWritableDatabase());
        Cursor cursorDB = db.rawQuery("SELECT * FROM " + TABLAPROFESORES + " WHERE " + PROFESOR_TUTOR + " = " + "'"+tutor+"';",null);
        return cursorDB;
    }
    //SELECT CICLO PROFESORES
    public Cursor devolverCicloProfesores(String ciclo){
        SQLiteDatabase db = (this.getWritableDatabase());
    ...
    public Cursor devolverAlumnosPorInicial(String nombre){
        ...
        String queryPorInicial = ("SELECT * FROM " + TABLAESTUDIANTES + " WHERE " + ESTUDIANTE_NOMBRE + " LIKE " + "'"+nombre+"%';");
        Cursor cursorDB = db.rawQuery(queryPorInicial,null);
        return cursorDB;
    }
    //SELECT POR INICIAL
    public Cursor devolverProfesoresPorInicial(String nombre){
        SQLiteDatabase db = (this.getWritableDatabase());

        String queryPorInicial = ("SELECT * FROM " + TABLAPROFESORES + " WHERE " + PROFESOR_NOMBRE + " LIKE " + "'"+nombre+"%';");
        Cursor cursorDB = db.rawQuery(queryPorInicial,null);
        return cursorDB;
    }
